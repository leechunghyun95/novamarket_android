 ...
 */
public class CustomDialog_nova extends Dialog {
    String TAG = "CustomDialog";
    private static CustomDialog_nova customDialog;

    private CustomDialog_nova(@NonNull Context context) {
        super(context);
    }

    public static CustomDialog_nova getInstance(Context context) {
            customDialog = new CustomDialog_nova(context);
    ...
    View.OnClickListener clickCancel = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            Log.d(TAG,"팀노바생활게시글 새로 작성하기 버튼 클릭");



            SharedPreferences pref = getContext().getSharedPreferences("팀노바생활", Activity.MODE_PRIVATE);
