...
public class AuthenticationActivity extends AppCompatActivity {
    EditText input_telephone;
    EditText input_code;
    Button sendCodeBtn;
    Button submitCodeBtn;
    TextView findbyEmailBtn;
    Context context;
    String code;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        submitCodeBtn = findViewById(R.id.submitCodeBtn);
        findbyEmailBtn = findViewById(R.id.findbyEmailBtn);
        
        //이메일로 계정찾기 버튼 클릭하면
        findbyEmailBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //이메일로 계정 찾기 액티비티로 이동
                startActivityC(AuthenticationWithEmailActivity.class);
            }
        });
        ...
        getSupportActionBar().setTitle("전화번호 인증"); // 툴바 제목 설정

        sendCodeBtn.setOnClickListener(new View.OnClickListener() {//인증문자 받기 버튼 클릭하면
            @Override
            public void onClick(View v) {
                if(input_telephone.getText().toString().length() == 11){//11자리 숫자를 입력하면 인증문자 발송하고 인증번호 입력창과 확인버튼 보이기.
        ...
        submitCodeBtn.setOnClickListener(new View.OnClickListener() {//인증번호 확인버튼 클릭하면
            @Override
            public void onClick(View v) {
                Log.d(TAG,"code: " + code);
                Log.d(TAG,"입력한 코드: " +input_code.getText().toString());
                if(code.equals(input_code.getText().toString())){//인증번호 맞을때
                    Log.d(TAG,"인증번호 일치");
                    //회원 DB에서 입력한 전화번호 있는지 검색
                            ...
                            if(status == NetworkStatus.TYPE_MOBILE || status == NetworkStatus.TYPE_WIFI) {
                                ...
                                public void onResponse(Call call, final Response response) throws IOException {
                                        ...
                                        if(responseData.equals("1")){//일치하는 전화번호가 회원 DB에 없으면
                                            Log.d(TAG,"회원가입 액티비티로 이동");
                                            Log.d(TAG,"telephone: " + telephone);
                                            startActivityString(SignUpActivity.class,"telephone",telephone);
                                        }else {
                                            Log.d(TAG,"메인액티비티로 이동");
                                            
                                            //로그인 시에 회원 전화번호랑 자동로그인값 트루로 SharedPreferences에 저장
                                            SharedPreferences pref = getSharedPreferences("pref", Activity.MODE_PRIVATE);
                                            SharedPreferences.Editor editor = pref.edit();
                                            editor.putString("telephone",input_telephone.getText().toString());
